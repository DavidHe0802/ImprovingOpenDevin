...
The thoughts before the most recent start action are thoughts for previous tasks, and the thoughts after it are for current task.
...
What is your next thought or action? Please focus on your main task.
...
def start_task(task: str):
    thought = {'action': 'start', 'args': {'thought': f"Now, I start a new task: {task}"}}
    return thought
